...
export const EoullimVote: React.FC<EoullimVoteProps> = ({ getVoteData, getVote }) => {
  ...
  const getTimeRemaining = (now: Date, end: Date) => {
  ...
  const onUpdate = () => {
    const now = new Date();
    if (endTime) {
      const time = getTimeRemaining(now, endTime);
  ...
  return (
    <S.EoullimVoteWrapper>
      <GoBackIcon style={{ marginLeft: 10 }} />
      <S.EoullimVoteContainer>
        <>
          <S.EoullimVoteHeader>
            <S.EoullimVoteStatusContainer style={{ marginTop: 30 }}>
